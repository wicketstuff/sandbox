...
/**
 * Component for embedding a jasper report in a page. This component must be attached to an &lt;object&gt; tag. If you
 * don't want to embed the report, but have a link to it instead, use {@link ResourceReference}.
 *
...
public final class EmbeddedJRReport extends WebComponent implements IResourceListener {
    ...
    public EmbeddedJRReport(MarkupContainer parent, String id, JRResource report) {
        super(parent, id);
        resource = report;
    }
    ...
    protected void onComponentTag(ComponentTag tag) {
        if(!"object".equalsIgnoreCase(tag.getName())) {
            findMarkupStream().throwMarkupException("Component " + getId() + " must be applied to a tag of type "
                + "'object' not " + tag.toUserDebugString());
        }
