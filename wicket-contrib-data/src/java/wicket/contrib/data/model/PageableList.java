 ...
 * count is used for the total number of elements that are available for this list, and
 * the window is used for the currently loaded elements.
 * <p>
 * This list figures out what elements need to be loaded when asked for elements. The
 * actual loading of the count and elements is delegated to the action objects.
 * </p>
 * <p>
 * Not all {@link java.util.List}methods are supported. Unsupported methods will throw
 * {@link java.lang.IllegalStateException}s.
 * </p>
 * <p>
 * NOTE: this list is meant for specific situations, like backing up
 * {@link wicket.markup.html.list.PageableListView}s, that use sequential addressing of
 * the list. Random access is inefficient, as a number of rows (the page) is loaded at
 * once for each window. Hence it IS efficent for the sequential access.
 * </p>
...
public class PageableList extends UnimplementedList
{
	 ...
	 * @param countAndListAction ction that selects a count and a (partial) list
	 */
	...
	/**
	 * Construct; uses the rowsPerPage property of the given table as the page size.
	 * @param table the table to get the page size from
	 * @param countAndListAction ction that selects a count and a (partial) list
	 */
	public PageableList(PageableListView table,
			ISelectCountAndListAction countAndListAction)
	{
		this(table.getRowsPerPage(), countAndListAction);
	}

	/**
	 * Clears the recorded count and the current loaded window; forces to reload the size/
	 * count and window.
	...
	/**
	 * Gets the element at the given position. If the index is outside of the currently
	 * loaded window, the window that the index should be in is loaded.
	 ...
	 * @param countAndListAction action that selects a count and a (partial) list.
	 */
	 ...
	 * @param windowSize size of the partial list that is to be actually loaded.
	 */
	...
	/**
	 * Gets the currently loaded items if any. This method will allways return
	 * a list, but when no elements are loaded yet, it will be empty.
	 * NOTE use this method for exceptional cases only; usually the normal
	 * list operations should suffice.
	...
	{
		Object result = countAndListAction.execute(getQueryObject());
		if (!(result instanceof Number))
		{
			throw new WicketRuntimeException("action result must be of type Number");
		}
		else
		{
	...
	/**
	 * Gets the object that holds the parameters for executing queries. This method just
	 * returns null.
	 ...
	 * 'startFromIndex' + 'numberOfElements' and return the results wrapped in a list.
	 * @param startFromIndex load from index
	 * @param numberOfElements the number of elements to load
	 * @return the results wrapped in a list
	 */
	protected List load(int startFromIndex, int numberOfElements)
	{
		return countAndListAction.execute(getQueryObject(), startFromIndex, numberOfElements);
	}
	...
	private final void loadPage(int startFromIndex, int numberOfElements)
	{
		...
		if (count > 0)
		{
			List loaded = load(startFromIndex, numberOfElements);
