...
public class TooltipPanel extends Panel
{
	...
	// Can be set to custom value if user's page requires tooltip to have higher
	// z-index.
	 ...
	 *            component id
	 * @param target
	 *            target component bound to Tooltip
	 */
	public TooltipPanel(MarkupContainer parent, Component target)
	{
		// set standardId for a tooltipPanel so user does not have to
		// pass 2 Id's when creating a tooltip
		super(parent, "tooltipPanel");
	 ...
	 *            target component bound to Tooltip
	 * @param model
	 *            Model to set for the Panel
	 */
	public TooltipPanel(MarkupContainer parent, IModel model, Component target)
	{
		// set standardId for a tooltipPanel so user does not have to
		// pass 2 Id's when creating a tooltip
		super(parent, "tooltipPanel", model);
	...
	/**
	 * constructor with custom x and y offsets NOTE: IE seems to take y as the
	 * offset from target's top and FF from target's bottom
	 * 
	 * @param id
	 *            Component id
	 * @param target
	 *            Target component bound to Tooltip
	 * @param x
	 *            X offset from target's upperleft corner
	 * @param y
	 *            Y offset from target's upperleft corner
	 * 
	 */
	public TooltipPanel(MarkupContainer parent, Component target, int x, int y)
	{
		// set standardId for a tooltipPanel so user does not have to
		// pass 2 Id's when creating a tooltip
		super(parent, "tooltipPanel");
	 ...
	 * 
	 * @param model
	 *            IModel the model of the component
	 * @param target
	 *            Target component bound to Tooltip
	 * @param x
	 *            X offset from target's upperleft corner
	 ...
	 *            Y offset from target's upperleft corner
	 * 
	 * NOTE: IE seems to take y as the offset from target's top and FF from
	 * target's bottom
	 */
	public TooltipPanel(MarkupContainer parent, IModel model, Component target, int x, int y)
	{
		// set standardId for a tooltipPanel so user does not have to
		// pass 2 Id's when creating a tooltip
		super(parent, "tooltipPanel", model);
	...
	{
		return new ResourceReference(TooltipPanel.class, "tooltip.js");
	}
	...
	/**
	 * innitializes the tooltip to be called from constructors to avoid
	 * redundant code.
	 */
	private void initTooltip()
	{
		...
		// add(new JavaScriptReference("tooltipMain", Tooltip.class,
		// "tooltip.js"));
		...
		add(new AppendAttributeModifier("style", true, new Model(
				"visibility:hidden; z-index:5; position:absolute")));

		// add mousehandler functions to target
		AppendAttributeModifier onMouseOverMod = new AppendAttributeModifier("onmouseover", true,
				new Model("xstooltip_show('" + getHTMLID() + "', '" + getTargetHTMLID() + "', "
						+ offsetX + ", " + offsetY + ", '" + getIFrameID() + "');"));
		AppendAttributeModifier onMouseOutMod = new AppendAttributeModifier("onmouseout", true,
				new Model("xstooltip_hide('" + getHTMLID() + "', '" + getIFrameID() + "');"));
		target.add(onMouseOverMod);
		target.add(onMouseOutMod);

		add(HeaderContributor.forJavaScript(getJs()));
	}
