...
/**
 * Class that can add fading functionality to a vissible Wicket Component
 * This class let's you set a trigger, a duration, a start opacity and
 * an end opacity.
 * 
 * @TODO usage examples
 * 
...
public class FXOnClickFader extends DojoFXHandler
{
	...
	/**
	 * convenience contructor with 2 parameters; 
	 * sets startDisplay default to false
	 * 
	 * @param duration duration of the fade effect
	 * @param trigger Component which ONCLICK method will be bound to trigger the fade effect.
	 */
	 ...
	 * @param duration duration of the fade effect
	 * @param trigger Component which ONCLICK method will be bound to trigger the fade effect.
	 * @param startDisplay default false; true if component should start with full opacity
	 */
	...
	/**
	 * Constructor for fade effect. If allowHide is true, component's display style will be ajusted on a fade.
	 * @param duration duration of the fade effect
	 * @param trigger Component which ONCLICK method will be bound to trigger the fade effect.
	 * @param startDisplay default false; true if component should start with full opacity
	 * @param allowHide if true component's display will be set to non when wiped-out, hence component will be removed from rendered page.
	 */
	...
	/**
	 * Constructor for a fade effect where the starting opacity and the ending opacity are set.
	 * @param duration duration of the fade effect
	 * @param trigger Component which ONCLICK method will be bound to trigger the fade effect.
	 * @param startDisplay default false; true if component should start with starting opacity (startOpac)
	 * @param startOpac starting opacity between 1 and 0. (0 for transparant, 1 for opaque)
	 * @param endOpac ending opacity between 1 and 0. (0 for transparant, 1 for opaque)
	 */
	...
	protected void renderHeadContribution(HtmlHeaderContainer container)
	{
		...
		if(type=="fadeHide")
		{
			fadeInFunction = "dojo.fx.html.fadeShow(node, duration, function(){"+ componentId  + "_faderState='fadedIn';})";
		    fadeOutFunction = "dojo.fx.html.fadeHide(node, duration, function(){"+ componentId  + "_faderState='fadedOut';})";
		} 
		else if(type == "fadeOpac") 
		{
			fadeInFunction = "dojo.fx.html.fade(node, duration,"+ startOpac+ "," + endOpac + ", function(){"+ componentId  + "_faderState='fadedIn';});";
		    fadeOutFunction = "dojo.fx.html.fade(node, duration,"+ endOpac+ "," + startOpac + ", function(){"+ componentId  + "_faderState='fadedOut';});";
		}
		else
		{
			fadeInFunction = "dojo.fx.html.fadeIn(node, duration, function(){"+ componentId  + "_faderState='fadedIn';})";
		    fadeOutFunction = "dojo.fx.html.fadeOut(node, duration, function(){"+ componentId  + "_faderState='fadedOut'});";
		}
	...
	protected void onBind()
	{
		...
		this.getTrigger().add(new AppendAttributeModifier(getEventName(),true, new Model(componentId + "_fade('"+ HTMLID +"', " + getDuration() + ");")));
