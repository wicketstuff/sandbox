	...
	public LiveSessionsPage() {
		...
		IModel<List<SessionData>> sessionModel = new Model<List<SessionData>>() {
			...
			public List<SessionData> getObject() {
				return new ArrayList<SessionData>(getRequestLogger()
						.getLiveSessions());
			}
		};
		PageableListView<SessionData> listView = new PageableListView<SessionData>(
				this, "sessions", sessionModel, 50) {
			private static final long serialVersionUID = 1L;
			...
			protected void populateItem(ListItem item) {
				...
				new Label(item, "requestCount", new Model<Integer>(new Integer(
						sd.getRequests().size())));
				new Label(item, "requestsTime", new Model<Double>(sd
						.getRequestsTime()));
				new Label(item, "sessionSize", new Model<Bytes>(Bytes.bytes(sd
						.getSessionSize())));
			}
		};
