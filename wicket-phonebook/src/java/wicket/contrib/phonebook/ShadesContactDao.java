...
public class ShadesContactDao implements ContactDao{
    ...
    public Iterator find(QueryParam qp, Contact filter) {        
        ...
        if(qp.isSortAsc())dbSess.setParameter("direction", "ASC");
        else dbSess.setParameter("direction", "DESC");
	/*
	if(qp.hasSort()){
            dbSess.setParameter("order", qp.getSort());
            if(qp.isSortAsc())dbSess.setParameter("direction", "ASC");
            ...
            q.clause("ORDER BY").disable();
            //dbSess.setParameter("order","");
