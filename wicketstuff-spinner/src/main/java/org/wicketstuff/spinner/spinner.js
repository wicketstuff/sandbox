 ...
 *
 * wicketstuff spinner
 * @author  Gerolf Seitz (gerolf.seitz at gmail dot com)
 * @date 2007-09-04
 * @license Creative Commons Attribution 3.0 (http://creativecommons.org/licenses/by/3.0/)
...
Wicket.Spinner.prototype = {
  ...
  observe: function() {
    ...
    var stop = this.stop.createDelegate(this);        
    // observe the input
    // begin incrementing at start of a keypress
    this.inputElement.onkeydown = function(evt) {this.keyStart(evt); }.createDelegate(this);    
   
   // stop incrementing at the end of a keypress       
   this.inputElement.onkeyup = this.stop.createDelegate(this);
   
    // reformat and enforce min-max for typed values 
    this.inputElement.onblur = function() { this.updateValue(0);}.createDelegate(this);
    
    // observe the up element
    // begin incrementing at start of click    
    this.upElement.onmousedown = function() { this.clickStart(1);}.createDelegate(this);
    
    // stop incrementing at end of click     
    this.upElement.onmouseup = function() { this.stop();}.createDelegate(this);
    
   // in the case of a click and drag, also stop     
    this.upElement.onmouseout = function() { this.stop();}.createDelegate(this);
      
    // observe the down element  
    // begin decrementing at start of click
    this.downElement.onmousedown = function() { this.clickStart(-1);}.createDelegate(this);
    // stop decrementing at end of click 
    this.downElement.onmouseup = function() { this.stop();}.createDelegate(this);
   // in the case of a click and drag, also stop 
    this.downElement.onmouseout = function() { this.stop();}.createDelegate(this);
  },
  /**
  ...
  increment: function() {
    this.updateValue(1);
    this.timeout = window.setTimeout(this.increment.createDelegate(this), this.getSpeed());
  ...
  decrement: function() {
    this.updateValue(-1);
    this.timeout = window.setTimeout(this.decrement.createDelegate(this), this.getSpeed());
