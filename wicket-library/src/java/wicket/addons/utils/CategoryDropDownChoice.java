...
public class CategoryDropDownChoice extends DropDownChoice
{
	...
	public CategoryDropDownChoice(final String id, final Long categoryId)
	{
		...
		final List categoryCount = ServiceLocator.instance().getCategoryService()
				.getAllCategories();
		...
		while (iter.hasNext())
		{
			...
			categoryOptions.add(new CategoryOption(cat.getName() + "  (" + cat.getAddonCount()
					+ ")", cat));
		}
		...
		for (Object data : categoryOptions)
		{
			...
			if ((option.getCategory() != null) && (option.getCategory().getId() != null)
					&& (categoryId != null)
					&& (option.getCategory().getId().doubleValue() == categoryId.doubleValue()))
			{
