 ...
 */
public final class AddonDaoSupport extends HibernateDaoSupport implements IAddonDao
{
    ...
    public final List getAddonsByUser(final int userId)
    {
        return getHibernateTemplate().find("from Addon a where a.owner.id=? ", new Integer(userId));
    }
    ...
    public List getAddonsByName(final String searchText, final int maxCount)
    {
        final HibernateTemplate hibernateTemplate = new HibernateTemplate(this.getSessionFactory());
        final String sqlStmt = "from Addon addon where name like ?";
