	...
	private void addComponents()
	{
		...
		this.table = new ListView("table", (List)null)
		{
			protected void populateItem(ListItem listItem)
			{
				...
				final String href = getPage().urlFor(NewAndUpdatedAddons.class,
						new PageParameters("category=" + category.getId())).toString();
				text += "<td width=\"50%\"><a class=\"lm\" href=\"" + href + "\">"
						+ category.getName() + "  (" + category.getAddonCount() + ")</a></td>";

				if ((mod != 0) || (categoryCount.size() == (listItem.getIndex() + 1))
						&& ((categoryCount.size() % 2) != 0))
				{
					text += "</tr>";
				}
		...
		final Link hrefAll = new BookmarkablePageLink("allHref", NewAndUpdatedAddons.class,
				new PageParameters("category=-1"));
		add(hrefAll);
