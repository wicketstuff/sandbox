...
public class TinyMCEInitializer implements IInitializer {
	...
	public void init(Application application) {
		...
		// TODO resource registration works recursively now;
		// consider using that instead of this customized search
		...
		if ("file".equals(protocol)) {
			...
			if (log.isErrorEnabled()) {
				log
						.error("unknown protocol, only file and jar are implemented");
			}
	...
	private void initFromJar(Application application) {
		...
		try {
			String basePath = scope.getResource(
					Strings.afterLast(scope.getName(), '.') + ".class")
					.getPath();
			String jarFilePath = new URL(Strings.beforeLast(basePath, '!'))
					.getPath();
