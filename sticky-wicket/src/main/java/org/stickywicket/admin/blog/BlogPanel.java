...
public class BlogPanel extends Panel {
	...
	private static final ResourceReference CSS = new ResourceReference(
			BlogPanel.class, BlogPanel.class.getSimpleName() + ".css");
	...
	public BlogPanel(MarkupContainer parent, String id) {
		...
		IDataProvider<Node> blogModel = new IDataProvider<Node>() {
			...
			public Iterator iterator(int first, int count) {
				try {
					NodeIterator i = JcrUtil.getRootNode().getNode("content")
							.getNodes("blog");
					i.skip(first);
					return i;
			...
			public int size() {
				try {
					return (int) JcrUtil.getRootNode().getNode("content")
							.getNodes("blog").getSize();
				} catch (RepositoryException e) {
					throw new RuntimeException(e);
				}
			}
		...
		DataView dataView = new DataView(this, "entry", blogModel) {
			...
			protected void populateItem(final Item item) {
				try {
					...
					Date date = node.getProperty("creationDate").getDate()
							.getTime();
					...
					new Label(item, "body", body.substring(0, Math.min(body
							.length(), 100)));
