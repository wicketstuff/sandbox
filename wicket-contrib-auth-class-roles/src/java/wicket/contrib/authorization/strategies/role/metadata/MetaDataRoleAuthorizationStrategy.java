...
import wicket.contrib.authorization.strategies.role.AbstractRoleAuthorizationStrategy;
...
public class MetaDataRoleAuthorizationStrategy extends AbstractRoleAuthorizationStrategy
{
   ...
   * @param roles Roles that are authorized to create component instances
   ...
   * @param componentClass
   *            The component type that is subject for the authorization
   * @param role Role that are authorized to create component instances
   *            of type componentClass
   */
  public static final void authorize(final Class< ? extends Component> componentClass,
    final Class role)
  {
    ...
    permissions.authorize(componentClass, new Roles(role));
  }
    ...
    final Class role)
  {
    ...
    permissions.authorize(action, new Roles(role));
  }
   ...
   * componentClass, Role.NO_ONE will automatically be added,
   ...
   *            The component type
   * @param roles
   *            The roles that are no longer to be authorized to create
   ...
   * this method. If the last authorization grant is removed for a given
   * action, Role.NO_ONE will automatically be added, effectively denying
   * access to all roles (if this was not done, all roles would suddenly
   * have access since no authorization is equivalent to full access).
   *
   * @param component
   *            The component
   * @param action
   *            The action
   * @param roles
   *            The roles that are no longer allowed to perform the given
   *            action
   */
   ...
   * Role.NO_ONE, effectively denying all other roles.
   *
   * @param componentClass
   *            The component class
   */
  public static final void unauthorizeAll(Class< ? extends Component> componentClass)
  {
    ...
    authorize(componentClass, Role.NO_ONE.class);
  }
  ...
  public static final void unauthorizeAll(final Component component, final Action action)
  {
    ...
    authorize(component, action, Role.NO_ONE.class);
  }
