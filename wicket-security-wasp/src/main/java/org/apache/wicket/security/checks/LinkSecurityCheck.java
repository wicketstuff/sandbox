 ...
 * link to be visible but disabled. Note that for all other actions this check behaves a
 * ClassSecurityCheck (with option to check the model).
 * @author marrink
 */
public class LinkSecurityCheck extends ComponentSecurityCheck
{
	 ...
	 * @param component the link
	 * @param clickTarget the {@link Page} redirected to when clicking on the link.
	 * @throws IllegalArgumentException if clickTarget is not a Page class
	 */
	public LinkSecurityCheck(AbstractLink component, Class clickTarget)
	{
		...
		if (!Page.class.isAssignableFrom(clickTarget))
		{
			throw new IllegalArgumentException("Class "
					+ clickTarget + " is not a subclass of Page");
		}
	}
	 ...
	 * @param component the link
	 * @param clickTarget the {@link Page} redirected to when clicking on the link.
	 * @param checkSecureModelIfExists forces the model to be checked after this check is
	 *            fired
	 * @throws IllegalArgumentException if clickTarget is not a Page class
	 */
	 ...
	 * @param useAlternativeRenderCheck true, if you want to use the alternative mode.
	 * @return this
	 */
