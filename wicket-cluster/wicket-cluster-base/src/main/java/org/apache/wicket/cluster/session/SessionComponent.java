...
public class SessionComponent implements MessageListener {
	...
	public void registerSessionProvider(SessionProvider sessionProvider) {
		contextPathToSessionProvider.put(sessionProvider.getContextPath(),
				sessionProvider);
	}
	...
	public SessionComponent(CommunicationModule communicationModule) {
		if (instance != null) {
			throw new IllegalStateException(
					"Only one instance of SessionComponent can be created");
		}
		instance = this;
	...
	private static final Logger log = LoggerFactory
			.getLogger(SessionComponent.class);

	private Boolean keepSessionAttributesSerialized = null;

	public Boolean isKeepSessionAttributesSerialized() {
		if (keepSessionAttributesSerialized == null) {
			keepSessionAttributesSerialized = Boolean
					.getBoolean("wjc-keep-session-attributes-serialized");
		}
		return keepSessionAttributesSerialized;
	}
